...
export class ArticleService {
  ...
  async update(id: number, updateArticleDto: CreateArticleDto): Promise<ArticleResponseDto> {
    ...
    // Проверяем уникальность title (если изменился)
  ...
  async delete(id: number): Promise<void> {
    ...
    // Получаем IDs маркеров этой статьи
    const markerIds = article.markers?.map(m => m.id) || [];
    
    // Удаляем статью
    await this.articleRepository.delete(id);
    
    // Проверяем каждый маркер, используется ли он в других статьях
    for (const markerId of markerIds) {
      ...
      // Если маркер не используется другими статьями, удаляем его
